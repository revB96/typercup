          ...
          for await (const [index, match] of Object.entries(schedule)) {
            await getTicketsStats(match._id).then(async (stats) => {
              ...
              if (stats.counter > 3) {
                ...
                if (stats.drawn)  statsDiv += `<div class="progress-bar-striped progress-bar-animated bg-warning" role="progressbar" style="width: ${stats.drawn}%" aria-valuenow="${stats.drawn}" aria-valuemin="0" aria-valuemax="100"><img src="img/handshake.png" /></div>`;
