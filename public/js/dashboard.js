          ...
          for await (const [index, match] of Object.entries(schedule)) {
            await getTicketsStats(match._id).then(async (stats) => {
              ...
              if (stats.counter > 3) {
                ...
                if (stats.t1) statsDiv += `<div class="progress-bar-striped progress-bar-animated bg-primary" role="progressbar" style="width: ${stats.t1}%" aria-valuenow="${stats.t1}" aria-valuemin="0" aria-valuemax="100"><span style="height: 8px; width: 8px;" class="flag-icon flag-icon-${match.t1.shortcut.toLowerCase()}"></div>`;
                ...
                if (stats.t2) statsDiv += `<div class="progress-bar-striped progress-bar-animated bg-info" role="progressbar" style="width: ${stats.t2}%" aria-valuenow="${stats.t2}" aria-valuemin="0" aria-valuemax="100"><span style="height: 8px; width: 8px;" class="flag-icon flag-icon-${match.t2.shortcut.toLowerCase()}"></div>`
