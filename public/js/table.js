    ...
    getUserTable().then(result=>{
        for (const [index,userStat] of Object.entries(result)) {
            ...
            <tr ${yellowClass}>
                ...
                <td>
                    <a href="#" style="font-color: black; text-decoration: none" title="${userStat.user.friendlyName}">${userStat.user.username}(<small>${userStat.user.friendlyName}</small>)${crown}</a>
                </td>
